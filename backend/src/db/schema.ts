...
export const users = pgTable("users", {
    id: text("id").primaryKey(), // providec by clerkId
    email: text("email").notNull().unique(),
    name: text("name"),
    imageUrl: text("image_url"),
    createdAt: timestamp("created_at", { mode: "date" }).notNull().defaultNow(),
    updatedeAt: timestamp("updated_at", { mode: "date" }).notNull().defaultNow(),
});
...
// 🔴 to automatically join related data when using `with: { realtionname: true }`

// 🔴 Users Realtions: A user can have many products and many comments
...
export const commentsRelations = relations(comments, ({ one }) => ({
    ...
    // `comments.productId` is the foreign key, `product.id` is the primary key
